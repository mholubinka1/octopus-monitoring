...
- Energy columns (`*_kwh`, `est_kwh`) → a custom unit of `kWh`. Where a panel's column names can't follow this suffix (e.g. the Consumption Heatmap's weekday-named columns below), apply the same `kWh` unit via an explicit field override instead of a name-pattern match.
...
`period_from >= NOW() - INTERVAL 45 DAY`, not 90 — `consumption`'s startup backfill only ever pulls `retention_days` (45) of history (see **Retention Window** in `.agent-docs/context.md`), so a 90-day window here would silently return less data than it claims, the same trap the file's other Row 2 panels are separately being fixed for. No completeness-guard `HAVING` clause is needed here, unlike the daily-total panels elsewhere in this file — each cell already averages across roughly twelve or thirteen same-weekday-same-hour samples spread over the window (`consumption` is half-hourly, so each hour bucket folds two readings per matching day, and 45 days spans roughly six or seven occurrences of each weekday), so one missing half-hour doesn't invalidate a whole cell the way it would invalidate a single day's total. A weekday/hour combination with zero matching rows in the window renders as an empty cell rather than a false zero — deliberate, matching how `AVG(CASE WHEN ...)` already behaves elsewhere in this file.
