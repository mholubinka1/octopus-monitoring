...
from data.mysql.client import MariaDBClient, _is_table_already_exists_error
...
def test_schema_sync_recovers_from_a_concurrent_table_creation_race(
    ...
    """Given app/ and hive_app/ (which shares octopus.job_run) start
    against a freshly-initialized database at the same moment, both can
    see that table as absent and race to create it -- the loser's CREATE
    TABLE fails "already exists". Constructing MariaDBClient (its public
    interface) must still succeed, recovering via a single retry, rather
    ...
    monkeypatch.setattr(
        "data.mysql.client.SQLBase.metadata.create_all", flaky_create_all
    )
...
def test_schema_sync_does_not_swallow_an_unrelated_schema_error(
    ...
    monkeypatch.setattr(
        "data.mysql.client.SQLBase.metadata.create_all", failing_create_all
    )
