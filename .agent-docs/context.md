...
**Home Monitoring**:
This repo's name (renamed from `octopus-monitoring`) now that it hosts more than one data-gathering container. Encompasses `octopus-app` and `hive-app`, sharing one MariaDB instance/database (still named `octopus` — see that term's entry for the planned `home_monitoring` rename) for downstream visualization (Grafana). Repo layout, already landed: `apps/` (deployable containers only — `octopus-app`, `hive-app`), `libs/` (`common`, no container of its own), `data/` (`grafana/`, `mariadb/`), `deployments/` (each app's Dockerfile and compose file, plus a combined top-level compose file — see **Combined Compose File**). Scoping tracked on a Wayfinder map ([#490](https://github.com/mholubinka1/home-monitoring/issues/490)). The GitHub repo rename itself landed via #496; the Docker Hub image rename and the database's own `octopus`→`home_monitoring` rename remain separate, still-open steps.
