...
- New weekly step `prune_old_data`, run as a continuation of `feature/yearly-consumption-comparison`'s `update_consumption_summary` job within the *same* scheduled `Job` and the *same* background worker thread — not as a second, independently-scheduled job. This is deliberate, not an implementation shortcut: `update_consumption_summary` dispatches via a retry-with-backoff background thread that returns before the work completes, so a separately-scheduled `prune_old_data` job firing on the identical `monday.at("04:00")` slot would have its gate check race the summary job's completion — it would observe whichever `job_run` happened to be most recently recorded (e.g. last week's success), not this cycle's real outcome. Sequencing them as one continuation makes that race structurally impossible: pruning only ever runs after summarization has genuinely finished, in the same thread.
...
- **Gating on aggregation success**: after `update_consumption_summary` completes (successfully or not, after exhausting its own retry-with-backoff attempts), `prune_old_data` checks `latest_job_run_is_successful("update_consumption_summary")`. If it wasn't a success, pruning is skipped for this cycle — logged, and recorded as its own `"skipped"` outcome via `job_run` — rather than risking deletion of raw data that hasn't yet been summarized. Implemented in `register_consumption_summary_job` (`app/main.py`), which composes two blocking, retry-with-backoff-wrapped, independently job_run-recording steps (`_with_backoff_recording`) into one `summarize_then_prune` function, dispatched via a single background worker thread (`_run_in_background`) — a decomposition of the prior `_schedule_refresh_job`/`_run_with_backoff_in_background` mechanism, which remains unchanged and still used as-is by every other scheduled job in this file.
