...
Everything else — retry/backoff strategy (genuinely different: `octopus-app`'s fixed-delay `retry()` vs `hive-app`'s `retry_with_exponential_backoff()`), exceptions, config schema (`ApplicationSettings` vs `HiveApplicationSettings`), and all domain-specific models/CRUD (consumption, pricing, cost forecast, heating status) — stays in its own app's package, unchanged in behavior.
