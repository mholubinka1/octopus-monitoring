...
- **Future tariff assumption**: the forecast assumes the account's *current* agreement/product continues unchanged through `billing_period_end` — it does not check for or react to a known future agreement with a later `valid_from` inside the billing period. A mid-period tariff switch (a rare, deliberate customer action) means the forecast silently continues pricing remaining days against the old tariff; not corrected for.
...
- New table `cost_forecast(id, billing_period_start, billing_period_end, actual_cost_to_date, projected_total_cost, computed_at)` — the app computes and persists both the actual-cost-to-date snapshot and the full-period projection together each run, so Grafana reads one row rather than needing to independently reconstruct billing-period boundaries. **`actual_cost_to_date` and `projected_total_cost` are stored already converted to GBP** — `product_rate.unit_rate`/`standing_charge` are pence/kWh and pence/day respectively (Octopus's own convention, never converted on ingest anywhere in this pipeline), so the calculation must divide by 100 before persisting. Grafana's own live queries (`grafana/mariadb/queries.md`) that compute cost directly from `product_rate` (not reading `cost_forecast`) need the same `/100` treatment — already corrected there for the panels that existed before this reconciliation.
- New daily job `cost_forecast_refresh` (explicitly *not* the hourly `refresh_interval_hours` cadence used by consumption/pricing refresh — billing periods span weeks, and there's no value in calling AgilePredict/Kraken more than once a day), using the same `job_run`-wrapped background-worker-thread mechanism established in `bugfix/consumption-timezone-and-scheduler-backoff`. "This billing period's cost so far" and "total expected cost" are therefore only as fresh as the last daily run — "yesterday's cost" is unaffected, since it's a separate live query with no dependency on this job.
