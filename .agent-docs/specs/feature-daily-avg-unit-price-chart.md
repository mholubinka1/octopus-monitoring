...
3. As the dashboard viewer, I want this panel visually grouped with Daily Average Cost and Daily Average Usage, so that the three related trends are easy to compare side by side.
...
- **Layout**: placed between Daily Average Cost and Daily Average Usage in the grid row at `y: 23` (new panel `id: 14`, `x: 10, w: 5`). Cost (`id: 8`) narrows from `x: 10, w: 7` to `x: 15, w: 4`; Usage (`id: 7`) narrows from `x: 17, w: 7` to `x: 19, w: 5`. All three continue to exactly fill the row's `x: 10` to `x: 24` span.
- **Incidental reconciliation**: while syncing against a fresh live export of the dashboard (which had drifted to `version: 36` from the repo's `version: 19`, 17 unreconciled manual edits), restored one regression found along the way — the "Latest Consumption" panel's cost query (id 4, query B) had reverted to the documented 88.9s-class range-predicate `product_rate` join; this was fixed back to the correlated-subquery form. All other live drift (new annotation markers, dashboard time range widened to `+120h`, several threshold/legend/display-mode tweaks) was carried over as-is. Full detail is in `queries.md`.
