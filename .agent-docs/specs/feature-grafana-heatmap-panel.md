...
- **Query shape**: transpose from long format (`day_of_week`, `hour_of_day`, `avg_kwh`) to wide format:
  ...
  - No completeness-guard `HAVING` clause — not needed here (each cell already averages ~6-7 same-weekday-same-hour samples spread across the window, unlike the daily-total panels the guard protects).
